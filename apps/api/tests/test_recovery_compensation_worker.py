...
from app.integrations.razorpay.payment_links import RazorpayPaymentLinkProvider
from app.services.recovery_compensation_batch import RecoveryCompensationBatchResult
from app.workers import recovery_compensation_worker
from app.workers.recovery_compensation_worker import (
    parse_run_once,
    run_recovery_compensation_worker,
)

NOW = datetime(2026, 8, 25, 21, 0, tzinfo=UTC)
CASE_ID = UUID("94000000-0000-0000-0000-000000000001")
...
def test_parse_run_once_defaults_to_continuous(
    ...
    monkeypatch.setattr(sys, "argv", ["recovery_compensation_worker"])
...
def test_parse_run_once_accepts_once_flag(
    ...
    monkeypatch.setattr(sys, "argv", ["recovery_compensation_worker", "--once"])
...
async def test_run_once_executes_batch_and_closes_database(
    ...
    provider = MagicMock(spec=RazorpayPaymentLinkProvider)
    session_factory = MagicMock(name="session_factory")
    run_batch = AsyncMock(
        return_value=create_batch_result(
            discovered_case_ids=(CASE_ID,),
        ),
    )
    close_database = AsyncMock()

    monkeypatch.setattr(
        recovery_compensation_worker,
        "get_settings",
    ...
    await run_recovery_compensation_worker(run_once=True)

    create_provider.assert_called_once_with(settings)
...
async def test_run_once_closes_database_after_batch_failure(
    ...
    monkeypatch.setattr(
        recovery_compensation_worker,
        "create_razorpay_payment_link_provider",
        MagicMock(return_value=MagicMock(spec=RazorpayPaymentLinkProvider)),
    )
    monkeypatch.setattr(
        recovery_compensation_worker,
        "get_session_factory",
...
async def test_missing_credentials_fail_before_database_initialization(
    ...
    with pytest.raises(RuntimeError, match="Key ID and Key Secret"):
        await run_recovery_compensation_worker(run_once=True)
...
async def test_continuous_empty_batch_sleeps_and_closes_on_cancellation(
    ...
    monkeypatch.setattr(recovery_compensation_worker.asyncio, "sleep", sleep)

    with pytest.raises(asyncio.CancelledError):
        await run_recovery_compensation_worker(run_once=False)
