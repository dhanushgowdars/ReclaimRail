...
export function LiveRecoveryCommand({ run, liveRun, polling, title, detail, safeError, pollError, webhookDelayWarning, isVerifiedReplay, copiedActionId, onCopyRecoveryLink, onOpenVerifiedReplay, onStartAnotherRun, approvingApprovalId, approvalError, onApprovalDecision }: LiveRecoveryCommandProps) {
  ...
  const heroDetail = isVerifiedReplay
    ...
    : outcome?.status === "recovered"
      ? "Razorpay reconciliation confirms recovered revenue for this run."
      : liveRun?.terminal
  ...
  return <section className="live-command" aria-live="polite" aria-label="Continuous live recovery workflow">
    ...
    <div className="live-command__layout live-command__layout--continuous">
      <section className="live-command__evidence" aria-labelledby="evidence-title"><div className="live-command__section-heading"><div><span>{isVerifiedReplay ? "Recorded recovery workflow" : "Continuous recovery workflow"}</span><h3 id="evidence-title">Watch evidence arrive in business order</h3></div><p>No step is animated or manually advanced. Entries appear only after the server records evidence.</p></div>
        {liveRun ? <ol className="live-command__timeline">{liveRun.steps.map((step, index) => <li ref={step.key === liveRun.active_step_key ? activeEventRef : undefined} className={`live-command__event is-${step.status}${step.key === liveRun.active_step_key ? " is-current" : ""}`} key={step.key}><span className="live-command__event-marker" aria-label={`Step ${index + 1}: ${statusLabel(step.status)}`}>{index + 1}</span><div><div className="live-command__event-topline"><div><span className="live-command__event-number">Step {index + 1}</span><strong>{step.label}</strong></div><em>{step.status === "active" ? <LoaderCircle className="spin" size={12} /> : null}{statusLabel(step.status)}</em></div><p>{step.detail}</p>{step.occurred_at ? <time dateTime={step.occurred_at}>{formatTimestamp(step.occurred_at)} IST{step.duration_milliseconds !== null ? ` · completed in ${formatDuration(step.duration_milliseconds)}` : ""}</time> : null}</div></li>)}</ol> : <div className="live-command__empty-evidence"><LoaderCircle className="spin" size={18} /> Checkout is ready. ReclaimRail is waiting for signed Razorpay evidence before beginning recovery.</div>}
      </section>
      <aside className="live-command__decision" aria-label="Recovery decision brief"><div className="live-command__decision-heading"><span><Sparkles size={15} /> Recovery decision brief</span><strong className={fallbackUsed ? "is-fallback" : ""}>{liveRun?.agent ? fallbackUsed ? "Deterministic fallback" : "Gemini trace" : "Awaiting evidence"}</strong></div><h3>{recommendedAction ? humanize(recommendedAction) : "No recovery action proposed yet"}</h3><p>{liveRun?.agent?.reasoning_summary ?? "Gemini can propose only after a signed failure is correlated. Deterministic policy remains the decision-maker."}</p>
        ...
        <dl className="live-command__decision-facts"><div><dt>{fallbackUsed ? "Verified failure evidence" : "What AI found"}</dt><dd>{decisionFinding ? humanize(decisionFinding) : "Waiting for signed evidence"}</dd></div><div><dt>Safety decision</dt><dd>{latestAction ? humanize(latestAction.policy_outcome) : "Waiting for deterministic policy"}</dd></div></dl>
