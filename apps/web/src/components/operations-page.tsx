...
export function OperationsPage({ section, summary, cases, outcomes, incidents }: { section: Section; summary: RecoveryDashboardSummary; cases: RecoveryCaseQueueItem[]; outcomes: RecoveryOutcome[]; incidents: RecoveryIncident[] }) {
  ...
  return <div className="app-shell"><RecoveryNavigation active={section} /><main className="workspace operations-page"><header className="operations-page__hero"><p className="kicker">{content.eyebrow}</p><h1>{content.title}</h1><p>{content.description}</p></header>
    ...
    {section === "outcomes" && <section className="panel"><div className="panel-heading"><div><p className="kicker">Financial outcome state</p><h2>Recovery evidence by reconciliation status</h2></div><CircleDollarSign size={25} /></div><OutcomeLedger outcomes={outcomes} currency={summary.currency} /></section>}
