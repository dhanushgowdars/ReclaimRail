...
type GlyphName = "grid" | "queue" | "shield" | "pulse" | "receipt";
...
function Glyph({ name }: { name: GlyphName }) {
  const paths = {
    ...
    receipt: <path d="M6 3h12v18l-3-2-3 2-3-2-3 2-3-2V3Zm3 5h6M9 12h6" />,
  };
      ...
      <nav aria-label="Command center navigation" className="navigation">
        <Link
          className={`navigation__item ${
            active === "overview" ? "navigation__item--active" : ""
          }`}
          href="/"
        >
          <Glyph name="grid" />
        ...
        <Link className="navigation__item" href="/#recovery-queue">
          <Glyph name="queue" />
          Recovery queue
        </Link>

        <Link className="navigation__item" href="/#outcomes">
          <Glyph name="receipt" />
          Outcome ledger
        </Link>

        <Link className="navigation__item" href="/#incidents">
          <Glyph name="pulse" />
          Rail incidents
        </Link>

        <Link className="navigation__item" href="/#safety-controls">
          <Glyph name="shield" />
          Safety controls
        </Link>
      </nav>
